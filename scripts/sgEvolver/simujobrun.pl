...
if( @ARGV < 4 ){
    die "Usage: simujobrun.pl <FastA input sequence> <GFF annotation file> <random seed> <simulation_dir> [debug]";
}

my $seq_filename = $ARGV[0];
my $gff = $ARGV[1];
my $simulation_dir = $ARGV[3];

my $simujobparams = "$simulation_dir/simujobparams.pm";
...
my $tree_path = "$simulation_dir/$simujobparams::tree_filename";
...
" --inversion-size=$simujobparams::inv_size $tree_path $seq_filename $seq_filename $simujobparams::evolved_seqs_name $simujobparams::evolved_seqs_fname";
