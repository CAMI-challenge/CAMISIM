...
'''
	...
	unknown_genomes_id_column_name = "ID"
	#otu_prediction_column_name = "OTU_NCBI_TAXONOMIC_PREDICTION"
	otu_id_column_name = "OTU_ID"
	...
	if otu_id_column is None:
		print("Meta Data file: OTU_ID column not found but required!")
		sys.exit(1)
	...
	with ANIm(input_querry_file, input_ref_file, None, nucmer_exe=nucmer_exe, logger=logger, pool_size=pool_size) as ani_calculator:
		otus = mothur_otus.get_otus(otu_cutoff_column, otu_id_column, unknown_genomes_id_column)
		#logger.info("OTUS: {}".format(otus))
		#sys.exit(0)
		for unknown_genomes_id in unknown_genomes_id_column:
			if otus[unknown_genomes_id] is None:
				continue
			ani_calculator.add_nucmer_cmd_lines(otus[unknown_genomes_id], [unknown_genomes_id])

		min_lengths, min_sim_errors, min_perc_ids, min_perc_aln, ncbi = ani_calculator.calculate_minimum_anim()
		...
		for row_index in range(0, number_of_unknown):
			...
			if unknown_genomes_id in min_perc_ids:
				if float(min_perc_ids[unknown_genomes_id]) > 0.98:
					ani_prediction_novelty_column[row_index] = "new_strain"
				elif float(min_perc_ids[unknown_genomes_id]) > 0.96:
					ani_prediction_novelty_column[row_index] = "new_species"
				ani_prediction_column[row_index] = ncbi[unknown_genomes_id]
				ani_column[row_index] = str(min_perc_ids[unknown_genomes_id])
