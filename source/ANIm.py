...
class ANIm:
	...
	def get_nucmer_cmd(self, reference_id, candidate_id, mum=True, maxmatch=False):
		...
		if maxmatch:
			...
			mode = "-mum"
		bash_prefix = "REF_FILE=`mktemp`;CAN_FILE=`mktemp`;tr -d '\\015' < {} > \"$REF_FILE\";tr -d '\\015' < {} > \"$CAN_FILE\";".format(self.reference_file_names[reference_id], self.candidate_file_names[candidate_id])
		bash_suffix = ";rm \"$REF_FILE\" \"$CAN_FILE\""
	...
	def multiprocessing_run(self, verbose=False):
		...
		"""
		self.logger.info("Running %d jobs with multiprocessing" % len(self.cmd_lines))
	...
	def calculate_minimum_anim(self):
		"""
		"""
		min_lengths, min_sim_errors, min_perc_ids, min_perc_aln, ncbi = {}, {}, {}, {}, {}
		...
		for candidate_id_reference_id in perc_ids:
			...
			if candidate_id not in min_perc_ids or min_perc_ids[candidate_id] < perc_ids[candidate_id_reference_id]:
