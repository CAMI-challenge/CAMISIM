...
def otu_prediction_to_meta_table(metadata_table_in, metadata_table_out, mothur_otu_file, threshold):
	executable = os.path.dirname(os.path.realpath(__file__)) + "/otu_prediction_to_meta_table.py"
	#print executable, "-i", metadata_table_in, "-o", metadata_table_out, "-cl", mothur_otu_file
	subprocess.call([executable, "-i", metadata_table_in, "-o", metadata_table_out, "-cl", mothur_otu_file, "-th", str(threshold)])
...
- working directory where the results will be saved
...
def gene_alignment_and_clustering(parser):
	"""The second step is to align 16S sequences using "mothur", which requires a high quality reference alignment (e.g. from SILVA). Also using "mothur", empty or uninformative columns are removed. The clustering will be done based on the distance between the sequences.
input:
- fasta formatted reference alignment (e.g. from SILVA)
- a choice of distance method
	0 -> phyml, using BioNJ algorithm (neighbor joining tree, 4000 taxa maximum!),
	1 -> raxml (maximum likelihood tree),
	2 -> mothur (like DNADist, but supposedly faster)
- a threshold (between 0 and 1), that will be used for the otu clustering. 0.03 is default.
...
- the number of processors that are available for paralell processing. The program "mothur" can use several cores for the alignment.
output:
- a mothur formatted file containing the otus, from unique up to the given threshold
"""
	args = parser.parse_args()
...
def classification_of_genomes_and_novelty_prediction(parser):
	"""The third step is to align 16S sequences using "mothur", which requires a high quality reference alignment (e.g. from SILVA). Also using "mothur", empty or uninformative columns are removed. The clustering will be done based on the distance between the sequences.
input:
- meta data table with a list of the genomes that are to be classified
- working directory where the results will be saved and which contains the mothur formatted file with the otus
output:
- meta data table with a list of the genomes, with columns added that contain otu based tax prediction, rank and novelty prediction
"""
	args = parser.parse_args()
	...
	#suffix16s = "16S_rRNA.fna"
	mothur_otu_file = working_directory + "/mothur_otu.txt"
	#otu_prediction_to_meta_table(metadata_table_in, metadata_table_out, mothur_otu_file, ncbitax_sqlite_db)

	otu_prediction_to_meta_table(metadata_table_in, metadata_table_out, mothur_otu_file, args.threshold)
	return True
...
"""
	...
	ani_prediction_to_meta_table(metadata_table_in, metadata_table_out, input_querry_file, input_reference_file, mothur_otu_file, pool_size)
	return True
...
FILE  /net/metagenomics/projects/cami_2014/otu_clustering/trunk/run.py

DESCRIPTION
Script for extracting 16S rRNA sequences from a set of genomes (new ones and reference genomes), clustering of 16S sequences for OTU determination (multipe alignment, distance matrix creation and OTU clustering), representing species. The clustering is then used to assign taxonomic id's to genomes of unknown origin, if they clustered with one or more of the reference genomes.
There are three stages:

1. Extraction of 16S sequences
2. Creation of a mulitple alignment of 16S sequences, distance matrix calculation and OTU clustering
...
- a threshold (between 0 and 1), that will be used for the otu clustering. 0.03 is default.
- meta data table with a list of the genomes that are to be classified
- working directory where intermediary results will be saved
- the number of processors that are available for paralell processing. The program "mothur" can use several cores for the alignment.
output:
- meta data table with a list of the genomes, with columns added that contain otu based tax prediction, rank and novelty prediction
...
- put all logfiles into working directory
"""
...
'''.format(sys.argv[0])
	...
	description = "Obtain otu cluster and genome classification based on 16S marker genes"
	...
	group.add_argument("-irf", "--input_reference_fna_file", default=None, type=str,
						help="path to a fasta file containing the 16S marker genes of the reference genomes")
	group.add_argument("-ir", "--input_reference_file", default=None, type=str,
	...
	parser.add_argument("-wd", "--working_directory", default=None, type=str,
						help="folder that will contain the found marker genes and also OTUs in mothur format")
	...
	parser.add_argument("-dist", "--distance_method", default=2, type=int,
						help='''available distance methods 0-2:
0 -> phyml BioNJ (4000 taxa maximum),
1 -> raxml,
2 -> mothur (no error correction)
Default: 2
''')
...
''')
	...
	parser.add_argument("-doc", "--doc_string", default=False, action='store_true',
						help="Display detailed description")
...
def print_doc_string():
	help(my_main)
	help(marker_gene_extraction)
	help(gene_alignment_and_clustering)
	help(classification_of_genomes_and_novelty_prediction)
