...
def main_transcriptome(directory="./"):
    dict_sequence_mapping = write_sam_transcriptome(directory)
    merge_fastq_transcriptome(directory, dict_sequence_mapping)
    return dict_sequence_mapping

def merge_fastq_transcriptome(directory, dict_sequence_mapping):
    
    fastq = os.path.join(directory, "*.fastq")
    old_fastq = sorted(glob.glob(fastq))[0]

    orig_file_prefix = os.path.basename(old_fastq).rsplit("_",1)[0]
    file_path = os.path.join(directory, orig_file_prefix + ".fq")
    with open(file_path, 'a+') as write_handler:

        for record in SeqIO.parse(old_fastq, "fastq-sanger"):
            record.id = dict_sequence_mapping[record.id]
            record.description = ""
...
# Change pbsim wgs to pbsim3 trans
def write_header_transcriptome(sam_file, sequence_ids):
...
def read_tsv_to_dict(tsv_file_path):
        ...
        with open(tsv_file_path, 'r') as file:
            ...
            for line in file:
                ...
                key_value_map[key] = [value1, value2]
...
def write_sam_transcriptome(directory):

    maf = os.path.join(directory, "*.maf")
    file_path = glob.glob(maf)[0]
    
    orig_file_prefix = os.path.basename(file_path).rsplit("_",1)[0]
    # orig_file_prefix = os.path.basename(file_path).rsplit(".",1)[0]
    sam_file = os.path.join(directory, orig_file_prefix + ".sam")

    fa = os.path.join(directory, "*.fa")
    ref_fasta = sorted(glob.glob(fa))[0]

    references = SeqIO.to_dict(SeqIO.parse(ref_fasta, "fasta"))

    write_header_transcriptome(sam_file, references)

    # Read the TSV file into a dictionary
    read_id_to_seq_id_dict = read_tsv_to_dict("./transcript_id_to_seq_id.tsv")
    ...
    sam_file = os.path.join(directory, orig_file_prefix + ".sam")
    with open(sam_file, "a") as samfile:
        dict_of_maf_sid, dict_read_counts = read_maf_transcriptome(samfile, read_id_to_seq_id_dict, dict_seq, dict_seq_quality, file_path)

    with open("count.tsv", 'w') as counts:
                for key in dict_read_counts:
                    counts.write(key + "\t" + str(dict_read_counts[key]) + "\n")
...
def read_maf_transcriptome(samfile, read_id_to_seq_id_dict, dict_seq, dict_seq_quality, file_path):

    dict_read_counts = dict()
    ...
    with open(file_path, "r") as maffile:
        ...
        for line in maffile:
            ...
            if line.startswith("a"):
                ...
                # maf_s = line.strip().split(" ")
                if n % 2 == 1:
                    sequence_id, start_gene = read_id_to_seq_id_dict[maf_s[1]]
                    POS = str(int(start_gene) + int(maf_s[2]))
                    SEQ_ref = maf_s[6]

                    if(maf_s[1] in dict_read_counts):
                        dict_read_counts[maf_s[1]] = dict_read_counts[maf_s[1]] + 1
                    else:
                        dict_read_counts[maf_s[1]] = 1
    ...
    return dict_of_maf_sid, dict_read_counts
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Generate SAM from maf file.')
    parser.add_argument('--transcriptome', action='store_true', help='Maf files are from transcriptomic simulation')
    args = parser.parse_args()

    from_transcriptome = args.transcriptome

    if(from_transcriptome):
        main_transcriptome()
    else:
        main()
