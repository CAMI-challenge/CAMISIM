...
def read_taxonomic_profile(biom_profile, no_samples=None):
    ...
    # number_of_samples is unused outside; leaving as local avoids global mutation
        ...
        with open(additional_file, 'r') as add:
            for line in add:
                ...
                if ncbi_id in genomes_map:
                    ...
                    genomes_map[ncbi_id] = (sci_name, [path], novelty)
                total_genomes += 1
    with open(genomes_path, 'r') as genomes:
        for line in genomes:
            ...
            if ncbi_id in genomes_map:
                ...
                genomes_map[ncbi_id] = (sci_name, [http], 'known_strain')
            total_genomes += 1
...
def get_genomes_per_rank(genomes_map, ranks, max_rank):
    ...
    for rank in ranks:
        per_rank_map[rank] = {}
    for genome in genomes_map:  # dict preserves insertion order (Python 3.7+)
        try:
            lineage = ncbi.get_lineage(genome)
            ranks_lin = ncbi.get_rank(lineage)
            for tax_id in lineage:  # keep NCBI lineage order
                try:
                    check_rank = ranks_lin[tax_id]
                except KeyError:
                    continue
                if check_rank in per_rank_map:
                    rank_map = per_rank_map[ranks_lin[tax_id]]
                    if tax_id in rank_map:
                        for strain in genomes_map[genome][1]:
                            rank_map[tax_id].append((strain, genome))
                    else:
                        rank_map[tax_id] = []
                        for strain in genomes_map[genome][1]:
                            rank_map[tax_id].append((strain, genome))
    ...
    for member in lineage:
        ...
        if name in mapping:
            ...
            if ncbi.get_rank([taxid])[taxid] in ranks:
                new_lineage.append(taxid)
        else:
            name2 = name.split()[0]
            mapping2 = ncbi.get_name_translator([name2])
            if name2 in mapping2:
                taxid = mapping2[name2][0]
                if ncbi.get_rank([taxid])[taxid] in ranks:
                    new_lineage.append(taxid)
...
def map_otus_to_genomes(profile, per_rank_map, ranks, max_rank, mu, sigma, max_strains, debug, no_replace, max_genomes):
    ...
    for otu in sorted_otus:
        ...
        for tax_id in lineage:  # lineage sorted ascending
            ...
            # draw number of strains (deterministic from seeded RNG)
            strains_to_draw = max((np_rand.geometric(2.0 / max_strains) % max_strains), 1)

            # MINIMAL FIX 1: keep deterministic order of sampled genomes (no sets)
            if len(available_genomes) >= strains_to_draw:
                used_indices = np_rand.choice(len(available_genomes), strains_to_draw, replace=False)
                used_genomes = [available_genomes[i] for i in used_indices]  # ordered
            else:
                # take all in their existing (list) order
                used_genomes = list(available_genomes)

            # (optional stable dedup without reordering)
            seen = set()
            used_genomes = [(p, g) for (p, g) in used_genomes if (p, g) not in seen and not seen.add((p, g))]

            genome_set_size += len(used_genomes)
            log_normal_vals = np_rand.lognormal(mu, sigma, len(used_genomes))
            sum_log_normal = float(sum(log_normal_vals)) if len(log_normal_vals) else 1.0
            ...
            for i, (path, genome_id) in enumerate(used_genomes):
                ...
                relative_abundance = log_normal_vals[i] / sum_log_normal if sum_log_normal > 0 else 0.0
                ...
                if no_replace:
                    # MINIMAL FIX 2: deterministic traversal when removing used genomes
                    for new_rank in RANKS:
                        if new_rank not in per_rank_map:
                            continue
                        # iterate taxids in sorted order for stability
                        for taxid2 in sorted(per_rank_map[new_rank].keys()):
                            lst = per_rank_map[new_rank][taxid2]
                            try:
                                idx = lst.index((path, genome_id))
                                lst.pop(idx)
                            except ValueError:
                                pass
...
def fill_up_genomes(otu_genome_map, unmatched_otus, per_rank_map, tax_profile, debug):
    ...
    # MINIMAL FIX 3: deterministic build order
    for rank in RANKS:
        if rank not in per_rank_map:
            continue
        for taxid in sorted(per_rank_map[rank].keys()):
            genomes.setdefault(taxid, [])
            for path, genome_id in per_rank_map[rank][taxid]:
                if path not in added_genomes:
                    genomes[taxid].append((path, genome_id))  # keep list order
                    added_genomes.add(path)

    if len(unmatched_otus) == 0:
    ...
    # MINIMAL FIX 4: deterministic consumption order
    for tax_id in sorted(genomes.keys()):
        for path, genome_id in genomes[tax_id]:
            curr_otu = unmatched_otus[otu_indices[i]]  # deterministic from seed
...
def write_config(otu_genome_map, genomes_map, no_samples, script, out_path_genomes):
    ...
    # Iterate insertion order of otu_genome_map (deterministic given upstream fixes)
    ...
    # Abundance file list string was only used for a removed config write; keeping side effect out.
...
if __name__ == "__main__":
    ...
    # Seed global NumPy RNG for deterministic draws
    np_rand.seed(seed)

    # run_patch()  # optional; disabled for stability
    ncbi = NCBITaxa()
    # ncbi.update_taxonomy_database()  # DO NOT update for reproducibility
    ...
    otu_genome_map, unmatched_otus, per_rank_map = map_otus_to_genomes(
        tax_profile, per_rank_map, RANKS, MAX_RANK, mu, sigma, max_strains, debug, no_replace, total_genomes
    )
